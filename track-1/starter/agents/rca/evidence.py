...
def render_evidence(case, decision, records, prediction, status, warnings, errors):
    ...
    for warning in dict.fromkeys([*warnings, *getattr(decision, "limitations", [])]):
    ...
    for record in shown:
        ...
        try:
            lines.append("  Observed / computed values: `" + _safe(_json(record.values)) + "`.")
            lines.append("  Units: `" + _safe(_json(record.units)) + "`.")
            lines.append("  Method parameters: `" + _safe(_json(record.transform_params)) + "`.")
        except (TypeError, ValueError):
            lines.append("  Invalid structured values could not be rendered as finite JSON.")
        for query in record.queries:
            lines.append(f"  Query `{_safe(query.source)}` [{_when(query.start)}, {_when(query.end)}); components={_safe(query.component_ids)}; operations={_safe(query.operation_names)}; KPIs={_safe(query.kpi_names)}.")
