...
from agents.rca.traces import TRACE_COLUMNS, inspect_dependencies, replay_evidence, triage_traces
...
class SyntheticStoreTests(unittest.TestCase):
    ...
    def test_independent_trace_candidates_native_units_and_replay(self):
        ...
        group = next(e for e in bundle.evidence if e.transform == "traces.group_compare.v1"
        ...
        health = next(e for e in bundle.evidence if e.transform == "traces.pairing_quality.v1")
    ...
    def test_row_cap_is_partial_and_replay_reproduces_retained_prefix(self):
        self.make_traces()
        with patch("agents.rca.traces.MAX_ROWS_PER_QUERY", 5):
            bundle = triage_traces(self.case, self.store, deadline=time.monotonic() + 10)
