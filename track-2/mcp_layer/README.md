...
- **[uv](https://docs.astral.sh/uv/)** in your PATH. `uv` resolves and caches the
  ...
  (Claude Desktop and Cursor run MCP servers in uv-managed environments already.)
- **The data prepped.** The server loads `data/prepped/` and `data/synthetic/` at
  import, exactly like the API. From the track-2 root:
...
```bash
uv run --with-requirements requirements.txt python -m mcp_layer.server
```

`--with-requirements requirements.txt` supplies `fastmcp` and the API's own deps
into an ephemeral, cached environment. Nothing is installed into your system
Python.
